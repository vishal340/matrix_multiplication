...
__global__ void gpuadd(float* A,int jump,float* B,int jump1,float* C,int jump2,int n1,int n2)
{
	int idx=blockIdx.x*blockDim.x+threadIdx.x;
	int idy=blockIdx.y*blockDim.y+threadIdx.y;
	if(idx<n1&&idy<n2)
		C[idx*jump2+idy]=A[idx*jump+idy]+B[idx*jump1+idy];
}

__global__ void gpusubtract(float* A,int jump,float* B,int jump1,float* C,int jump2,int n1,int n2)
{
	int idx=blockIdx.x*blockDim.x+threadIdx.x;
	int idy=blockIdx.y*blockDim.y+threadIdx.y;
	if(idx<n1&&idy<n2)
...
__global__ void gpumultiply(float* A,int jump,float* B,int jump1,float* C,int jump2,int n1,int n2,int n3)
{
	int idx=blockIdx.x*blockDim.x+threadIdx.x;
	int idy=blockIdx.y*blockDim.y+threadIdx.y;
	if(idx<n1&&idy<n3)
	{
		float sum=0;
		for(int k=0;k<n2;k++)
			sum+=A[idx*jump+k]*B[k*jump1+idy];
		C[idx*jump2+idy]+=sum;
	}
}
