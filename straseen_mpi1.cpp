...
int main(int argc,char** argv){
    ...
    const int n1[3]={n[0]-temp[0],n[1]-temp[1],n[2]-temp[2]};
    const int m1[3]={(int)((n[0]-temp[0])/(iter[0]*iter[1])),(int)((n[1]-temp[1])/iter[1]),(int)((n[2]-temp[2])/(iter[0]*iter[1]))};

    float* matrix_loc1=new float[m[0]*m[1]]();   //matrix A for local processor
    int row=m1[0]+((n1[0]%(iter[1]*iter[0])) > dim_rank[0] ? 1:0); //no. of rows for output matrix from this processor
    int cont=m1[1]+(((n1[1]%iter[1])>dim_rank[1]) ? 1:0);
    //std::cout<<row<<'\t'<<cont<<'\t'<<rank<<'\n';

    int offset1=(((n1[0]%(iter[1]*iter[0])) > dim_rank[0] ? dim_rank[0]:(n1[0]%(iter[1]*iter[0])))+dim_rank[0]*m1[0])*n1[1]+\
                (((n1[1]%iter[1])>dim_rank[1]) ? dim_rank[1]:(n1[1]%iter[1]))+dim_rank[1]*m1[1];
    MPI_File matrix1;
    MPI_File_open(MPI_COMM_WORLD,argv[1],MPI_MODE_RDONLY,MPI_INFO_NULL,&matrix1);
    //std::cout<<row<<'\t'<<cont<<'\t'<<offset1<<'\t'<<rank<<'\n';
    MPI_Datatype new_float1;
    MPI_Type_contiguous(cont,MPI_FLOAT,&new_float1);
    MPI_Type_commit(&new_float1);
    for(int i=0;i<row;i++){
      MPI_File_set_view(matrix1,2*sizeof(int)+sizeof(float)*(i*n1[1]+offset1), \
                      MPI_FLOAT,new_float1,"native",MPI_INFO_NULL);
      MPI_File_iread(matrix1,matrix_loc1+i*m[1],cont,MPI_FLOAT,&request);
      MPI_Wait(&request,&status);
    }
    /*MPI_File_close(&matrix1);
    std::cout<<matrix_loc1[0]<<'\t';

    float* matrix_loc2=new float[m[1]*m[2]]();    //matrix B for local processor
    int col=m1[2]+((n1[2]%(iter[1]*iter[0])) > dim_rank[0] ? 1:0); //no. of col for output matrix from this processor

    int offset2=((n1[2]%(iter[1]*iter[0])) > dim_rank[0] ? dim_rank[0] : (n1[2]%(iter[1]*iter[0])))+ \
                m1[2]*(int)(dim_rank[0]/iter[1])*iter[1]+n1[2]*((((n1[1]%iter[1])>dim_rank[1]) ? dim_rank[1]:(n1[1]%iter[1]))+\
                  dim_rank[1]*m1[1])+((dim_rank[0]%iter[1]+dim_rank[1])%iter[1])*m1[2];
    MPI_File matrix2;
    MPI_File_open(MPI_COMM_WORLD,argv[2],MPI_MODE_RDONLY,MPI_INFO_NULL,&matrix2);
    //MPI_Datatype new_float2;
    //MPI_Type_contiguous(col,MPI_FLOAT,&new_float2);
    //MPI_Type_commit(&new_float2);
    for(int i=0;i<cont;i++){
      //MPI_File_set_view(matrix2,2*sizeof(int)+sizeof(float)*(i*(n[2]-temp[2])+offset2), \
                      MPI_FLOAT,new_float2,"native",MPI_INFO_NULL);
      MPI_File_iread(matrix2,matrix_loc2+i*m[2],col,MPI_FLOAT,&request);
      MPI_Wait(&request,&status);
    }
