...
int main(int argc,char** argv){
  ...
  if(iter[1]>1){
    ...
    if(iteration[1]>0){
      ...
      float* temp;
      const int source4=dim_rank[1]<iteration[1]? ((dim_rank[1]+1)%iteration[1]+dim_rank[0]*iter[1]):rank;
      const int dest4=dim_rank[1]<iteration[1]? ((dim_rank[1]+iteration[1]-1)%iteration[1]+dim_rank[0]*iter[1]):rank;
      const int source5=dim_rank[1]<iteration[1]? (dim_rank[1]!=(iteration[1]-1) ? ((dim_rank[1]+1)%iteration[1]+((dim_rank[0]+iter[0]-1)%\
                        iter[0])*iter[1]):(((dim_rank[0]+iteration[1]-1)%iter[0])*iter[1])):(((dim_rank[0]+1)%iter[0])*iter[1]+dim_rank[1]);
      const int dest5=dim_rank[1]<iteration[1]? (dim_rank[1]!=0 ? ((dim_rank[1]+iteration[1]-1)%iteration[1]+((dim_rank[0]+1)%\
                        iter[0])*iter[1]) : (iteration[1]-1+((dim_rank[0]+iter[1]-iteration[1]+1)%iter[0])*iter[1])) : \
      ...
      for(int i=0;i<iteration[1];i++){
        ...
        temp=new float[((int)(m[0]/row_size)+(m[0]%row_size>row_rank ? 1:0))*m[2]];
        for(int j=0;j<row_size-1;j++){
          MPI_Sendrecv(matrix_loc3[iteration[0]]+(((row_size+row_rank-j-1)%row_size)*(int)(m[0]/(row_size))+\
                  ...
                  ((int)(m[0]/row_size)+(m[0]%row_size>((row_size+row_rank-j-1)%row_size)?1:0))*m[2],\
                    MPI_FLOAT,(row_size+row_rank-j-1)%row_size,0,temp,((int)(m[0]/row_size)+(m[0]%row_size>\
                    row_rank?1:0))*m[2],MPI_FLOAT,(row_rank+1)%row_size,0,row_comm,&status);
      ...
      for(int i=0;i<iteration[0]+(dim_rank[1]<iteration[1]?1:0);i++){
        ...
        for(int j=0;j<row;j++){
          MPI_File_iwrite_at(matrix3,2*sizeof(int)+sizeof(float)*((gap[2]>shift?shift*(m1[2]+1):(gap[2]+shift*m1[2]))+\
                            (gap[0]>dim_rank[1]? dim_rank[1]*(m1[0]+1)*n[2]:(gap[0]+dim_rank[1])*n[2])),
